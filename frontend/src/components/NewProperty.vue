    ...
    <div style="width:80%;">
        <el-form :model="property" label-width="180px">
            ...
            <el-form-item label="Type">
                <el-select v-model="property.type" placeholder="Select" size="default">
                    <el-option
                        v-for="item in ['INTEGER', 'BIGINT', 'FLOAT', 'DOUBLE', 'DECIMAL', 'TEXT', 'STRING', 'BOOLEAN', 'DATE', 'DATEONLY', 'UUID']"
                        :key="item"
                        :label="item"
                        :value="item"
                    />
            ...
            <el-form-item>
                <el-button type="primary" @click="createOrUpdate">{{ isCreate ? 'Create': 'Update' }}</el-button>
...
onMounted(async () => {
    if (!isCreate.value) {
        ...
        property.value = {
            ...pro,
            defaultValue: pro.defaultValue === null ? '' : JSON.parse(pro.defaultValue).toString(),
            validate: pro.validate === null ? '' : JSON.stringify(JSON.parse(pro.validate), undefined, 2),
        }
    }
})
...
const createOrUpdate = async () => {
    ...
    const propertyToCreate = {
        ...property.value,
        defaultValue: getDefaultValue(property.value.type, property.value.defaultValue),
        validate
    }
    if (!isCreate.value) {
