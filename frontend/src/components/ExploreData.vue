<template>
  <div style="min-width: 90%">
    <el-page-header @back="back" style="margin-bottom: 20px">
        <template #content>
            <span class="text-large font-600 mr-3"> Explore data of {{ model.name }} in the database {{ model.dbName }} </span>
        </template>
    </el-page-header>
    <div style="margin-bottom: 10px;">
        <el-row>
            ...
            <el-link v-show="selected!=null" type="primary" :underline="false" @click="update">Update</el-link>
            <el-text v-show="selected!=null">&nbsp;or&nbsp;</el-text>
            <el-link v-show="selected!=null" type="danger" :underline="false" @click="del">delete</el-link>
            <el-text v-show="selected!=null">&nbsp;the selected item.</el-text>
        </el-row>
    </div>
    <div style="margin-bottom: 30px;">
        <el-table stripe :data="list" :border="true" empty-text="No data" highlight-current-row @current-change="e=>selected=e" >
            <el-table-column
                ...
                <template #default="scope">
                    ...
                    <template v-else-if="pro.type==='DATE' || pro.type==='DATEONLY'">
                        <el-tag size="small" type="warning">{{scope.row[pro.name]}}</el-tag>
                    </template>
                    <template v-else>
...
const create = async e => {
    ...
    isCreate.value = true
    editorData.value = JSON.stringify(loadExampleObject(true, properties.value), undefined, 2)
}

const update = async e => {
    dialogVisible.value = true
    isCreate.value = false
    editorData.value = JSON.stringify(loadExampleObject(false, properties.value, selected.value), undefined, 2)
}
...
const createOrUpdate = async () => {
    ...
    if (isCreate.value) {
        ...
        await toastAction(async () => {
            await rpc.expdata.update(model.value.id, JSON5.parse(editorData.value))
        }, 'update data')
        list.value = await rpc.expdata.get(model.value.id)
    }
}
