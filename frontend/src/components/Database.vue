...
<div style="min-width: 50%">
  <div style="margin-bottom: 30px;">
    <el-row>
      <el-text v-show="list.length===0">Currently there're no defined databases.&nbsp;</el-text>
  ...
  <div style="margin-bottom: 30px;" v-for="element in list" :key="element.name">
    <el-descriptions :title="element.name" size="default" border style="margin-bottom: 10px;">
      ...
      <el-descriptions-item label="Size">{{ element.size }} bytes</el-descriptions-item>
...
const create = async e => {
  let name = null
  try {
    name = await ElMessageBox.prompt('Please input the database name', 'Create database', {
